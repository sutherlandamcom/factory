import { GapAuthorityReader } from "./authority.js";
import {
...
export class CompetitorContentGapService {
  ...
  async evaluateUpstreamStaleness(
    ...
    return new GapAuthorityReader(this.deps).evaluateUpstreamStaleness(projectId, bound);
  }

  private async evaluateAcceptedSnapshotStaleness(projectId: string, snapshot: AcceptedContentGapSnapshotRecord) {
    return new GapAuthorityReader(this.deps).evaluateAcceptedSnapshotStaleness(projectId, snapshot);
  }
