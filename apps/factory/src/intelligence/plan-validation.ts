...
function topicRelatesToServiceInputs(
  ...
  for (const candidate of [...serviceSeeds, ...mustCoverServices]) {
    const normalizedCandidate = normalizeScalarText(candidate);
    if (normalizedCandidate.length === 0) continue;
    if (normalizedTopic.includes(normalizedCandidate) || normalizedCandidate.includes(normalizedTopic)) {
      return true;
    }
  }
  return false;
...
export function validateSiteIntelligencePlan(
  ...
  for (const mustCover of mustCoverServices) {
    const normalizedMustCover = normalizeScalarText(mustCover);
    const covered = servicePages.some((page) => {
      const normalizedTopic = normalizeScalarText(page.primaryTopic);
      return normalizedTopic.includes(normalizedMustCover) || normalizedMustCover.includes(normalizedTopic);
    });
    if (!covered) {
  ...
  const factTexts = (request.business.operatorFacts ?? []).map((fact) => fact.text);
  for (const page of servicePages) {
    ...
    if (topicSupportedByText(page.primaryTopic, factTexts)) continue;
    addIssue(
      `service page "${page.slug}" primary topic "${page.primaryTopic}" is unsupported by serviceSeeds, mustCoverServices, operator facts, or referenced evidence`,
    );
  }
