...
- **External Design Provider**: creates professional visual design candidates. Preferred v0 provider is Google Stitch behind a provider adapter; cheaper/alternative providers are tested only if the preferred provider fails the agreed quality floor. The provider owns generation only and is never Factory design authority; human acceptance produces the immutable/version-bound `AcceptedDesignArtifact`, which is the durable Factory design authority.
...
Professional visual direction and page design are generated by an external `DesignProvider`. Google Stitch is the preferred first v0 candidate because of current cost/integration fit; Framer, Figma and other challengers are evaluated only if the preferred provider fails the quality floor.
...
Provider output is a proposal/evidence until human acceptance. Factory stores/version-binds the exact accepted provider-derived artifact and approval as `AcceptedDesignArtifact`; that artifact, not Stitch/provider IDs, HTML or screenshots, is the durable Factory design authority. Generated HTML/design-system material may be implementation input; it is not automatically trusted production output.

Current `SiteBlueprint.designDirection`, SiteProductionSpec `creativeDirection`, and legacy `design_director` evaluation roles are transitional planning/evaluation constructs. They do **not** supersede the external DesignProvider generation role or the accepted `AcceptedDesignArtifact` authority.
...
Google Stitch / `DesignProvider` remains the professional design **generation provider**, not Factory authority. Human acceptance creates the immutable/version-bound `AcceptedDesignArtifact`, which is the Factory design authority. Stitch/provider `DESIGN.md`, screenshots and raw HTML may be retained as implementation/design evidence, but they are not production content, asset, renderer or acceptance authority.
...
8. External `DesignProvider` generates professional design candidates; human acceptance creates `AcceptedDesignArtifact`, the durable Factory design authority. Factory does not build a proprietary design brain.
